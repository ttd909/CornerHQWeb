...
3. **The CornerHQ app.** Real member app screenshot `images/member-app.png` (Sing Muay Thai, cream and gold) plus the admin mock `images/admin-dashboard.png`, recoloured to the same palette and re-shot from `preview/index.html`.
...
| 6 | What we do | heading, then a row of four | Heading "What we do", lead "Four things, done for one industry. Buy one or all of them." Four columns: Promo videos, Marketing, Websites, Apps. Each: a 4:5 still from the reel on top, title, one sentence. No card boxes. Two columns on mobile. (Changed from a vertical list on 2026-09-04 at the owner's request: the list looked bare.) |
